...
def run_test_largest_number():
    ...
    expected = 7
    answer = largest_number(([1], [2], [3], [4,5], [6,7]))
    print('Expected and actual are:', expected, answer)
...
def largest_number(seq_seq):
    ...
    for k in range(len(seq_seq)):
        for b in range(len(seq_seq[k])):
            if type(seq_seq[k][b]) == int:
                if number == False:
                    number = seq_seq[k][b]
                if seq_seq[k][b] > number:
                    number =seq_seq[k][b]
    if number == False:
        return "none"
    else:
...
def run_test_largest_negative_number():
    ...
    # ------------------------------------------------------------------
    # ODO: 4. Implement this TEST function.
...
def first_is_elsewhere_too(seq_seq):
    ...
    for k in range(len(seq_seq[0])):
        for b in range(len(seq_seq)-1):
            for m in range(len(seq_seq[b+1])):
                if seq_seq[b+1][m] == seq_seq[0][k]:
                    return True
    return False
